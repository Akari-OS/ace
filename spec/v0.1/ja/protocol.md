...
`bypass:` ブロックは、次のトップレベルディレクティブ行または空行が現れるまで続く。
...
編集上のパッチ。「Appendix B — Open issues」の全5件を解消または v0.2 以降に明示的に移管。
...
> v0.1.1 編集パッチにて全5件の Open issues を解消した。
> 実装判断の見直しが必要な事項は以下のとおり v0.2 以降に移管する。

1. **`allow:` の複数エントリ競合時の評価順序**（v0.2 で検討）
   - v0.1 では「すべての `deny:` より後に評価される」と規定するが、`allow:` 同士が重なる場合の詳細な優先度は未規定。
   - v0.2 で「後方優先（最後の `allow:` が勝つ）」または「最も具体的パターン優先」を正式決定する。

2. **エージェントごとのアクセス制御**（v0.2 で検討）
   - パス単位ではなくエージェント識別子（`@claude-code`、`@cursor`）を主キーとするアクセス制御は、
     ケイパビリティ言語を新設せずには表現困難なため v0.2 以降に先送りする。

3. **セマンティック lint の信頼度スコア正規化**（v0.2 で検討）
   - 異なる LLM ベンダー間でスコアを比較可能にする正規化手法は、
     十分なベンチマークデータが揃うまで規定できない。v0.2 で安定化する。
